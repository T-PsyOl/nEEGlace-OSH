...
// Function to compute spectral centroid from a buffer using FFT
float computeSpectralCentroid(const std::vector<float>& buffer, int sampleRate) {
...
bool setup(BelaContext *context, void *userData) {
    ...
    if (recordAudio == 1) {
        ...
        csvFilename = generateFilename("onset_detection") + ".csv";
        // Open the CSV file for writing
        csvFile.open(csvFilename);
        if (csvFile.is_open()) {
            // Write the header to the CSV file
            csvFile << "timestamp,detection,RMS,frequency_content\n";
            csvFile.flush();
            csvFileOpen = true;
...
void render(BelaContext *context, void *userData) {
    ...
    for (unsigned int n = 0; n < context->audioFrames; ++n) {
        ...
        if (hopCounter >= hopSize) {
            ...
            float freqContent = computeSpectralCentroid(audioBuffer, context->audioSampleRate);
            ...
            if (detection) {
                onsetBuffer.push_back({timestamp, energy, rms, freqContent});
        ...
        if (recordAudio == 1) {
            // Store the current audio sample in the audio recorder buffer
            audioRecorder[0][recordingFrames] = currentSample;
            // Store the current digital pin state in the digital pin recorder buffer
            digitalPinRecorder[recordingFrames] = digitalRead(context, n, digitalPin);
            // Increment the recording frames counter
            ++recordingFrames;
            // If the recording is complete, stop the audio processing
